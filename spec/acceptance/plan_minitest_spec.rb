...
describe 'plan test::role with minitest' do
  ...
  describe 'test_tool=minitest' do
    it 'fails to run' do
      plan_result = run_plan('test::role', '', '', '', "target" => "webserver.local", "test_params" => {"test_tool" => "serverspec",  "test_file" => "webserver.rb" }, "ctrl_params" => { "tmp_dir" => "/Users/Shared/tmp" })
      expect(plan_result[0]['status']).to eq('failure')
    end
    it 'returns helpful error message' do
      plan_result = run_plan('test::role', '', '', '', "target" => "webserver.local", "test_params" => {"test_tool" => "serverspec" }, "ctrl_params" => { "tmp_dir" => "/Users/Shared/tmp" })
